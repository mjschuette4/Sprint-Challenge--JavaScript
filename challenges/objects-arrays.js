...
const universities = [];
for(ph in graduates) {
  universities.push(graduates[ph].university);
}
universities.sort();
...
const contactInfo = [];
for(ph in graduates) {
  contactInfo.push(`${graduates[ph].first_name} ${graduates[ph].email}`);
}
...
const unisWithUni = [];
for(ph in graduates) {
  if(graduates[ph].university.match('Uni')) {
    unisWithUni.push(graduates[ph].university);
  }
}
...
zooAnimals.forEach(name => {
  displayNames.push(`names: ${name.animal_name}, scientific: ${name.scientific_name}`)
});
